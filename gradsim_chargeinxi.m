...
% save directory
plots_dir           = ['gradsim_test/chargeinxi/',''];
plot_name_suffix    = [''];
save_format         = {'png','eps','fig'};
plot_name           = 'chargeinxi_3dir';
...
for d = 1:length(datadirs)
    ...
    for n = 1:length(dump_list)
        ...
        for x = 1:length(dephasing_xi)
            ...
            chargevsz(x,d,n) = O.cylindrical_integration(O.r(r_ind),O.z(z_ind),O.(O.species)(r_ind,z_ind),'trapz');
    ...
    O.progress_dump('xi',x,length(dephasing_xi))
end % for datadirs
...
for x = 1:length(dephasing_xi)
    ...
    for d = 1:length(datadirs)
        ...
        %         end
        hold on
        plot(squeeze(plot_z(x,d,:)),squeeze(chargevsz(x,d,:)),...
...
legend(ax(1),leg,'location','northeast','FontSize',10)
xlabel(tt,'z (m)')
