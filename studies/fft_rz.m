...
% data directory
datadirs = {'gp20'};
plots_dir = ['fft/rz/grads/',datadirs{1},'trans'];
P = Plotty();
...
low_freqrange = 0.97; % min frequency to plot (norm. to plasma freq) %0.89, 0.97
upp_freqrange = 1.12; % max frequency to plot (norm. to plasma freq) %1.02, 1.1
max_dotsize = 350;
...
AFFT = AwakeFFT(...
    ...
    'direction',direction,'wakefields_direction',wakefields_direction,...
...
%     title('g = -20 %')
    ...
    P.plot_name = ['fvsrz',AFFT.datadir,'n',num2str(dump_list(end))];
    P.fig_handle = fvsrz;
